...
import type { UserProfile, UserProfileProject, Event, Photoshoot } from '@/app/types/models';
...
function getSocialLink(socialLinks: { platform: string; url: string }[], platform: string): string | null {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-white/[0.04] border border-white/[0.07] rounded-[14px] overflow-hidden animate-pulse">
...
function Pagination({
  ...
  isLoading: boolean;
  t: { common: { previous: string; next: string; page: string; of: string } };
}) {
...
export function ProfilePublicView({ slug, initialData }: ProfilePublicViewProps) {
  const { t } = useLocale();
  ...
  // Fetch profile — skipped if initialData was provided
  useEffect(() => {
    if (initialData) {
      setIsFollowing(initialData.is_following || false);
      return;
    }
  ...
  // Fetch events (lazy — only when tab is active)
  useEffect(() => {
  ...
  // Fetch photoshoots (lazy — only when tab is active)
  useEffect(() => {
  ...
  const goToPrevious = useCallback(() => {
    ...
    if (!shoot) return;
    setSelectedPhotoIndex(selectedPhotoIndex === 0 ? shoot.images.length - 1 : selectedPhotoIndex - 1);
  ...
  const goToNext = useCallback(() => {
    ...
    const shoot = photoshoots.find((s) => s.id === selectedPhotoshootId);
    if (!shoot) return;
    setSelectedPhotoIndex(selectedPhotoIndex === shoot.images.length - 1 ? 0 : selectedPhotoIndex + 1);
  }, [selectedPhotoIndex, photoshoots, selectedPhotoshootId]);
  ...
  const getEventStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'past': return 'bg-white/10 text-white/40 border-white/20';
      default: return 'bg-white/10 text-white/60 border-white/20';
    }
  };
  const getEventStatusLabel = (status: string) => {
    switch (status) {
      case 'upcoming': return t.profile.upcoming;
      case 'ongoing': return t.profile.ongoing;
      case 'past': return t.profile.past;
  ...
  // ── Loading state (SSR skeleton) ──
  if (isLoadingProfile) {
    return (
      <div className="animate-pulse space-y-6 py-4">
        <div className="h-48 md:h-64 bg-white/[0.04] rounded-none -mx-4 sm:-mx-6 lg:-mx-8" />
        <div className="h-8 bg-white/[0.04] rounded w-1/3" />
        <div className="h-4 bg-white/[0.04] rounded w-1/4" />
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 bg-white/[0.04] rounded-[12px]" />
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="aspect-[4/3] bg-white/[0.04] rounded-[14px]" />
          ))}
        </div>
      </div>
    );
  ...
  const supportUrl = profile.ko_fi_url || profile.patreon_url;
  const supportLabel = profile.ko_fi_url ? t.profile.supportKofi : t.profile.supportPatreon;

  const tabs: { key: TabType; label: string; count: number; icon: React.ReactNode }[] = [
    { key: 'projects', label: t.profile.projects, count: profile.stats.public_projects_count, icon: <Folder className="w-4 h-4" /> },
  ];
  if ((profile.stats.photoshoots_count ?? 0) > 0) {
    tabs.push({ key: 'photoshoots', label: t.profile.photoshoots, count: profile.stats.photoshoots_count ?? 0, icon: <Camera className="w-4 h-4" /> });
  }
  if (profile.stats.events_count > 0) {
    tabs.push({ key: 'events', label: t.profile.events, count: profile.stats.events_count, icon: <Calendar className="w-4 h-4" /> });
  }
  ...
  return (
    <div className="flex-1">
      <div className="space-y-6">
        ...
        <div className="relative">
          <div className="h-48 md:h-64 relative overflow-hidden">
            {profile.has_cover && profile.cover ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-[#6259CA]/30 to-[#2BD1C7]/30" />
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-[#1E1A40] via-[#1E1A40]/50 to-transparent" />
          </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8">
            <div className="flex items-end gap-6">
              ...
              <div className="relative shrink-0">
                <div className="w-28 h-28 md:w-36 md:h-36 rounded-2xl overflow-hidden border-4 border-[#1E1A40] shadow-2xl bg-[#2a2550]">
                  {profile.has_avatar && profile.avatar ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-[#6259CA] to-[#2BD1C7] flex items-center justify-center">
                ...
                {/* Open to Commission badge */}
                {profile.open_to_commission && (
                  <div
                    className="absolute -bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap px-2 py-0.5 bg-[#2BD1C7] text-[#1E1A40] rounded-full font-semibold"
                    style={{ fontSize: '0.62rem', letterSpacing: '0.02em' }}
                  >
                    {t.profile.openToCommission}
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 pb-2 min-w-0">
                <h1 className="text-white text-2xl md:text-3xl font-bold truncate" style={{ letterSpacing: '-0.025em' }}>
                  {profile.name}
                </h1>
                {profile.country && (
                  <div className="flex items-center gap-1 text-white/60 mt-1">
                    <MapPin className="w-4 h-4 shrink-0" />
                    <span style={{ fontSize: '0.85rem' }}>{profile.country.name}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 pb-2 shrink-0 flex-wrap">
                {supportUrl && (
                  <a
                    ...
                    className="hidden sm:flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#FF5e5b] hover:bg-[#e84e4b] text-white font-medium transition-all hover:-translate-y-px"
                ...
                <ProfileLikeButton
                  slug={slug}
                  initialCount={profile.likes_count ?? 0}
                  initialLiked={profile.is_liked_by_user ?? null}
                />
                <button
                  onClick={handleFollow}
                  className={`px-5 min-h-[44px] rounded-xl font-medium transition-all ${
                    isFollowing
                      ? 'bg-white/10 text-white border border-white/20 hover:bg-white/20'
                      : 'bg-[#6259CA] text-white hover:bg-[#6259CA]/90'
                  }`}
                ...
                <button
                  onClick={handleShare}
                  className={`p-2.5 min-h-[44px] rounded-xl border transition-colors ${
                    showCopied
                      ? 'bg-[#2BD1C7] text-[#1E1A40] border-[#2BD1C7]'
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-center gap-6 justify-center lg:justify-start">
                <div className="text-center">
                  <p className="text-white text-2xl font-bold font-mono" style={{ letterSpacing: '-0.04em' }}>
                    {profile.stats.public_projects_count}
                  </p>
                  ...
                  <>
                    {websiteUrl && (
                      <a href={websiteUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-white hover:bg-white/10 transition-colors" title={t.profile.website}>
                        <LinkIcon className="w-5 h-5" />
                      </a>
                    )}
                    {instagramUrl && (
                      <a href={instagramUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-pink-400 hover:bg-pink-500/10 transition-colors" title={t.profile.instagram}>
                        <InstagramIcon className="w-5 h-5" />
                      </a>
                    )}
                    {twitterUrl && (
                      <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title={t.profile.twitter}>
                        <TwitterIcon className="w-5 h-5" />
                      </a>
                    )}
                    {youtubeUrl && (
                      <a href={youtubeUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-red-500 hover:bg-red-500/10 transition-colors" title={t.profile.youtube}>
                        <YoutubeIcon className="w-5 h-5" />
                      </a>
                    )}
                    {tiktokUrl && (
                      <a href={tiktokUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-white hover:bg-white/10 transition-colors" title={t.profile.tiktok}>
                        <TiktokIcon className="w-5 h-5" />
                      </a>
                    )}
                    {twitchUrl && (
                      <a href={twitchUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl bg-white/5 text-white/60 hover:text-purple-400 hover:bg-purple-500/10 transition-colors" title={t.profile.twitch}>
        ...
        {/* ── Ko-fi button (mobile only, header hides it on sm+) ── */}
        {supportUrl && (
          <div className="px-8 sm:hidden">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-2 py-3 bg-[#FF5e5b] hover:bg-[#e84e4b] text-white rounded-xl font-medium transition-all"
              style={{ fontSize: '0.85rem' }}
            >
              ☕ {supportLabel}
            </a>
        ...
        {/* ── Personal Records (Story 3.3) ── */}
        {profile.personal_records !== undefined && (
          <div className="px-8">
            <PersonalRecordsSection records={profile.personal_records} />
          </div>
        )}
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${
                  activeTab === tab.key
                    ? 'bg-[#6259CA] text-white'
                    : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'
                }`}
              >
                {tab.icon}
                <span>{tab.label}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === tab.key ? 'bg-white/20' : 'bg-white/10'}`}>
                  {tab.count}
                </span>
              </button>
            ))}
            ...
            <div>
              {isLoadingProjects ? (
                ...
                <>
                  ...
                  {projectsMeta && (
                    <Pagination meta={projectsMeta} onPageChange={setProjectsPage} isLoading={isLoadingProjects} t={t} />
                  )}
                </>
              ) : (
                <div className="text-center py-16 bg-white/[0.04] border border-white/[0.07] rounded-2xl">
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {photoshoots.map((shoot) => (
                      <div
                        key={shoot.id}
                        className="group bg-white/[0.04] border border-white/[0.07] rounded-[14px] overflow-hidden hover:border-[#2BD1C7]/50 transition-all cursor-pointer"
                      ...
                      >
                        <div className="aspect-[4/3] relative overflow-hidden">
                          ...
                          <div className="absolute top-3 right-3">
                            <span className="px-2 py-1 rounded-[6px] bg-[#2BD1C7]/15 text-[#2BD1C7] text-xs border border-[#2BD1C7]/30">
                  ...
                  {photoshootsMeta && (
                    <Pagination meta={photoshootsMeta} onPageChange={setPhotoshootsPage} isLoading={isLoadingPhotoshoots} t={t} />
                  )}
                </>
              ) : (
                <div className="text-center py-16 bg-white/[0.04] border border-white/[0.07] rounded-2xl">
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {events.map((event) => (
                      <div
                        key={event.id}
                        className="group bg-white/[0.04] border border-white/[0.07] rounded-[14px] overflow-hidden hover:border-purple-500/50 transition-all"
                      >
                        <div className="flex">
                          ...
                          <div className="flex-1 p-4 flex flex-col justify-between">
                            <div>
                              <div className="flex items-start justify-between gap-2 mb-2">
                                ...
                                <span className={`shrink-0 px-2 py-1 rounded-[6px] text-xs border ${getEventStatusColor(event.status)}`}>
                            ...
                            {event.website_url && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1 text-[#6259CA] text-sm hover:underline mt-2"
                              >
                  ...
                  {eventsMeta && (
                    <Pagination meta={eventsMeta} onPageChange={setEventsPage} isLoading={isLoadingEvents} t={t} />
                  )}
                </>
              ) : (
                <div className="text-center py-16 bg-white/[0.04] border border-white/[0.07] rounded-2xl">
          ...
          >
            ...
            <div className="absolute top-4 left-4 flex items-center gap-3">
              ...
              <div className="px-3 py-2 rounded-full bg-white/10 backdrop-blur-sm">
                <span className="text-white/70 text-sm">{selectedPhotoIndex + 1} / {images.length}</span>
              </div>
            </div>
            {images.length > 1 && (
              <button
                ...
                {images.map((img, index) => (
                  <button
                    ...
                    className={`shrink-0 w-16 h-16 rounded-lg overflow-hidden transition-all ${
                      index === selectedPhotoIndex ? 'ring-2 ring-[#6259CA] scale-110' : 'opacity-60 hover:opacity-100'
                    }`}
                  >
                    <ImageWithFallback
