...
export function AvatarUploadModal({
  ...
  const displayImage = previewUrl || (hasAvatar ? currentAvatar : null);
      ...
      <div className="relative bg-secondary border border-white/10 rounded-2xl w-full max-w-md p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-white mb-2">
            {t.account.profile.avatarUpload.title}
          </h2>
          <p className="text-white/60">
            {t.account.profile.avatarUpload.subtitle}
          </p>
        </div>
        ...
        <div className="mb-6">
          <div className="relative w-48 h-48 mx-auto">
            {displayImage ? (
              ...
              <div className="w-full h-full rounded-full bg-gradient-to-br from-primary to-tertiary flex items-center justify-center border-4 border-white/10">
        ...
        <div className="mb-6">
          ...
          >
            ...
            {selectedFile
              ? t.account.profile.avatarUpload.changeImage
              : t.account.profile.avatarUpload.selectImage}
          </button>
          {selectedFile && (
        ...
        <div className="mb-6 p-4 bg-white/5 rounded-lg border border-white/10">
          <p className="text-white/60 text-sm mb-2">
            {t.account.profile.avatarUpload.requirements}
          </p>
          <ul className="text-white/40 text-xs space-y-1">
            <li>• {t.account.profile.avatarUpload.formatRequirement}</li>
            <li>• {t.account.profile.avatarUpload.sizeRequirement}</li>
            <li>• {t.account.profile.avatarUpload.dimensionRequirement}</li>
          </ul>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {t.account.profile.avatarUpload.cancel}
          </button>
          <button
            type="button"
          ...
          >
            {isUploading ? (
              <>
                ...
                {t.account.profile.avatarUpload.uploading}
              </>
            ) : (
              t.account.profile.avatarUpload.upload
            )}
          </button>
        </div>
      </div>
